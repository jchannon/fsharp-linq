...
let linq11() =
    ...
    productInfos
    |> List.iter
        (fun x -> printfn "%s is in the category %s and costs %f per unit." x.ProductName x.Category x.Price)
...
let linq19() =
    ...
    let customerOrders =
        ...
        |> List.collect (fun (index, x) ->
            if isNull x.Orders then
                []
            else
                List.ofSeq x.Orders
                |> List.map (fun f2 -> sprintf "Customer #%i has an order with OrderId %i" (index + 1) f2.OrderId))
